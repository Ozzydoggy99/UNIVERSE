...
/**
 * Registers the specific pickup from 104 workflow routes
 * This workflow picks up a bin from shelf 104 and drops it at the main dropoff point
 */
export function registerPickupFrom104WorkflowRoute(app: express.Express) {
  /**
   * This handler creates a workflow for picking up a bin from 104
   * and delivering it to the main dropoff point
   */
  const handlePickupFrom104Workflow = async (req: express.Request, res: express.Response) => {
    const startTime = Date.now();
    
    logRobotTask(`🔄 Starting Pickup from 104 workflow`);
    ...
    try {
      ...
      // This is the shelf 104 point where we'll pick up the bin
      ...
      logRobotTask(`- Pickup (104): ${pickupPoint.id} at (${pickupPoint.x}, ${pickupPoint.y}), ori: ${pickupPoint.ori}`);
      ...
      // STEP 1: Go to 104 docking position first (approach point)
      workflowSteps.push({
        type: 'move',
        params: {
      ...
      // STEP 5: Go to dropoff point with to_unload_point for precise positioning
      workflowSteps.push({
  ...
  app.post('/api/pickup-from-104/workflow', handlePickupFrom104Workflow);
  
  logRobotTask('✅ Registered pickup-from-104 workflow handler');
}