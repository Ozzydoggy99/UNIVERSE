...
import { User, BoxMapPoint, insertBoxMapPointSchema } from "@shared/schema";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Box Map Point endpoints
  // Get all box map points
  app.get("/api/box-mappings", async (req, res) => {
    try {
      if (!req.isAuthenticated()) {
        return res.status(401).json({ message: "Authentication required" });
      }
      
      const boxMapPoints = await storage.getAllBoxMapPoints();
      return res.json(boxMapPoints);
    } catch (error) {
      console.error("Error fetching box map points:", error);
      return res.status(500).json({ message: "Error fetching box map points" });
    }
  });
  
  // Get all floors for a specific service type
  app.get("/api/box-mappings/floors/:serviceType", async (req, res) => {
    try {
      if (!req.isAuthenticated()) {
        return res.status(401).json({ message: "Authentication required" });
      }
      ...
      // Extract unique floors for the given service type
      const floors = new Set<number>();
      boxMapPoints.forEach(point => {
        if (point.serviceType === serviceType) {
          floors.add(point.floor);
        }
      });
      
      // Always include floors 1-10 (default floors)
      for (let i = 1; i <= 10; i++) {
        floors.add(i);
      }
      
      return res.json(Array.from(floors).sort((a, b) => a - b));
    } catch (error) {
      console.error("Error fetching floors:", error);
      return res.status(500).json({ message: "Error fetching floors" });
    }
  });
  
  // Get box map points by service type and floor
  app.get("/api/box-mappings/service/:serviceType/floor/:floor", async (req, res) => {
    try {
      if (!req.isAuthenticated()) {
        return res.status(401).json({ message: "Authentication required" });
      }
      ...
      if (!serviceType || isNaN(floor)) {
        return res.status(400).json({ message: "Service type and floor are required" });
      }
      
      const boxMapPoints = await storage.getBoxMapPointsByServiceAndFloor(serviceType, floor);
      return res.json(boxMapPoints);
    } catch (error) {
      console.error("Error fetching box map points by service and floor:", error);
      return res.status(500).json({ message: "Error fetching box map points" });
    }
  });
  
  // Get box map point by ID
  app.get("/api/box-mappings/:id", async (req, res) => {
    try {
      if (!req.isAuthenticated()) {
        return res.status(401).json({ message: "Authentication required" });
      }
      ...
      const id = parseInt(req.params.id);
      const boxMapPoint = await storage.getBoxMapPoint(id);
      
      if (!boxMapPoint) {
        return res.status(404).json({ message: "Box map point not found" });
      }
      
      return res.json(boxMapPoint);
    } catch (error) {
      console.error("Error fetching box map point:", error);
      return res.status(500).json({ message: "Error fetching box map point" });
    }
  });
  
  // Create box map point
  app.post("/api/box-mappings", async (req, res) => {
    try {
      if (!req.isAuthenticated() || req.user.role !== 'admin') {
        return res.status(403).json({ message: "Only admins can create box map points" });
      }
      ...
      const newBoxMapPoint = await storage.createBoxMapPoint(createData);
      return res.status(201).json(newBoxMapPoint);
    } catch (error) {
      console.error("Error creating box map point:", error);
      return res.status(500).json({ message: "Error creating box map point" });
    }
  });
  
  // Update box map point
  app.put("/api/box-mappings/:id", async (req, res) => {
    try {
      if (!req.isAuthenticated() || req.user.role !== 'admin') {
        return res.status(403).json({ message: "Only admins can update box map points" });
      }
      
      const id = parseInt(req.params.id);
      const boxMapPoint = await storage.updateBoxMapPoint(id, req.body);
      ...
      console.error("Error updating box map point:", error);
      return res.status(500).json({ message: "Error updating box map point" });
    }
  });
  
  // Delete box map point
  app.delete("/api/box-mappings/:id", async (req, res) => {
    try {
      if (!req.isAuthenticated() || req.user.role !== 'admin') {
        return res.status(403).json({ message: "Only admins can delete box map points" });
      }
      
      const id = parseInt(req.params.id);
      const success = await storage.deleteBoxMapPoint(id);
      
      if (!success) {
        return res.status(404).json({ message: "Box map point not found" });
      }
      
      return res.json({ message: "Box map point deleted successfully" });
    } catch (error) {
      console.error("Error deleting box map point:", error);
      return res.status(500).json({ message: "Error deleting box map point" });
    }
  });
  
  // Move robot to a specific box point
  app.post("/api/axbot/navigate-to-box/:id", async (req, res) => {
    try {
      if (!req.isAuthenticated()) {
        return res.status(401).json({ message: "Authentication required" });
      }
      ...
      // If in demo mode, immediately return fallback data
      if (isDemoMode) {
        console.log(`Using demo mode to navigate to box point ID: ${id}`);
        return res.json({
          ...FALLBACK_CONTROL_RESPONSE,
          message: `Navigating to ${boxMapPoint.serviceType} unit ${boxMapPoint.unitNumber} on floor ${boxMapPoint.floor} (Demo Mode)`
        });
      }
      
      try {
        // Send command to AxBot API to navigate to specific coordinates
        const response = await axios.post(`${API_ENDPOINT}/control/navigate`, {
          x: boxMapPoint.xCoordinate,
          y: boxMapPoint.yCoordinate,
          z: 0, // Assuming ground level
          description: `${boxMapPoint.serviceType.charAt(0).toUpperCase() + boxMapPoint.serviceType.slice(1)} Unit ${boxMapPoint.unitNumber}`
        }, {
          headers: {
            "Authorization": `Bearer ${API_KEY}`,
            "Content-Type": "application/json"
        ...
        console.error("Error navigating to box point:", error);
        // Return fallback data
        return res.json({
          ...FALLBACK_CONTROL_RESPONSE,
          message: `Navigating to ${boxMapPoint.serviceType} unit ${boxMapPoint.unitNumber} on floor ${boxMapPoint.floor} (Demo Mode)`
      ...
      console.error("Error navigating to box point:", error);
      return res.status(500).json({ message: "Error processing navigation request" });
    }
  });
