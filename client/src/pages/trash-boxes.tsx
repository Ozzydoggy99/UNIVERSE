...
interface TrashBoxesProps {
  user: User | null;
}

export default function TrashBoxes({ user }: TrashBoxesProps) {
        ...
        try {
          ...
          if (response.ok) {
            ...
            if (templateData && templateData.layout) {
              ...
              // Find the trash component and get its floors setting
              const trashComponent = layout.components.find((comp: any) => 
                comp.type === 'rectangle' && comp.icon === 'trash'
              );
              
              if (trashComponent && trashComponent.floors) {
                setNumFloors(Math.min(Math.max(1, trashComponent.floors), 99));
              }
            }
          }
        } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 relative">
      ...
      <div className="pt-16 pb-8">
        <h1 className="text-2xl font-bold text-center mb-4">Trash Control</h1>
